...
# modify visual plot change with slider and add side-by-side comparison "Live vs. Full"
# app_v2_live.py — CGT Supply Chain Simulator (Autologous with Substeps + Live/Full Comparison)
# -----------------------------------------------------------------------------------
# This revision adds a side-by-side "Live vs Full" comparison panel while keeping
# automatic recompute and persistent plots. Live mode downsamples for speed; Full mode
# runs the exact inputs. KPIs and Gantt charts are shown for both, plus a delta table.
# -----------------------------------------------------------------------------------
...
class ScenarioParams:
    ...
    vector_delay_prob: float
    vector_delay_mean: float
    vector_delay_sd: float
...
class CGTSim:
    def __init__(self, p: ScenarioParams):
        ...
        self.mfg = simpy.Resource(self.env, capacity=p.mfg_rigs)
    ...
    def patient_flow(self, idx: int):
        ...
        # Apheresis
        yield self.env.process(self._do_step(self.apheresis, lambda: truncated_gauss(s['Apheresis'].mean, s['Apheresis'].sd), pid, 'Apheresis'))
        # Ship-In
        start = self.env.now
        ...
        # Activation
        yield self.env.process(self._do_step(self.mfg, lambda: truncated_gauss(s['Activation'].mean, s['Activation'].sd), pid, 'Activation'))
        # Vector wait
        ...
        # Transduction
        yield self.env.process(self._do_step(self.mfg, lambda: truncated_gauss(s['Transduction'].mean, s['Transduction'].sd), pid, 'Transduction'))
        # Expansion
        yield self.env.process(self._do_step(self.mfg, lambda: truncated_gauss(s['Expansion'].mean, s['Expansion'].sd), pid, 'Expansion'))
        # Fill/Finish
        start = self.env.now
        ...
        # QC
        yield self.env.process(self._do_step(self.qc, lambda: truncated_gauss(self.p.qc_days, s['QC'].sd), pid, 'QC'))
        # Release
        start = self.env.now
...
# -------------------------
# Streamlit UI (Live vs Full)
# -------------------------

st.set_page_config(page_title="CGT Simulator v2.2 — Live vs Full", layout="wide")
st.title("Cell & Gene Therapy Supply Chain Simulator — Autologous v2.2 (Live vs Full)")

with st.sidebar:
    st.header("Live Settings")
    live_mode = st.toggle("Recompute on change", value=True)
    live_patients_cap = st.slider("Max patients in live mode", 5, 120, 30, step=5)
...
# Build two scenarios: Full (exact) and Live (downsampled if enabled)
...
if live_mode:
    ...
    live_h = float(live_horizon)
else:
    live_params = full_params
    live_h = 365.0

# Cache layer
@st.cache_data(show_spinner=False)
def cached_run(params: ScenarioParams, horizon: float, key: int):
    sim = CGTSim(params)
...
# Keys to invalidate cache on parameter change
def make_key(p: ScenarioParams, horizon: float) -> int:
    tup = (
        p.n_patients, p.interarrival_days, p.apheresis_capacity, p.mfg_rigs, p.qc_stations, p.clinic_slots_per_day,
        p.steps['Apheresis'].mean, p.steps['Apheresis'].sd,
        p.steps['Ship-In'].mean, p.steps['Ship-In'].sd,
        p.steps['Activation'].mean, p.steps['Activation'].sd,
        p.steps['Transduction'].mean, p.steps['Transduction'].sd,
        p.steps['Expansion'].mean, p.steps['Expansion'].sd,
        p.steps['Fill/Finish'].mean, p.steps['Fill/Finish'].sd,
        p.qc_days,
        p.steps['Release'].mean, p.steps['Release'].sd,
        p.steps['Ship-Out'].mean, p.steps['Ship-Out'].sd,
        p.steps['Infusion'].mean, p.steps['Infusion'].sd,
        p.vector_delay_prob, p.vector_delay_mean, p.vector_delay_sd,
        p.target_lead_time_days, p.seed, horizon
    )
    return hash(tup)

# Run both scenarios
ref_date = pd.Timestamp('2025-01-01')
live_key = make_key(live_params, live_h)
full_key = make_key(full_params, 365.0)

live_events, live_lt, live_kpis = cached_run(live_params, live_h, live_key)
full_events, full_lt, full_kpis = cached_run(full_params, 365.0, full_key)

# --- Comparison layout ---
st.subheader("Live vs Full Comparison")
colL, colR = st.columns(2)

with colL:
    st.markdown("**Live (downsamped for speed)**")
    c1, c2 = st.columns(2)
    c1.metric("Throughput", f"{live_kpis['throughput']}")
    c2.metric("On‑Time (%)", f"{live_kpis['on_time_pct']:.0f}%" if not math.isnan(live_kpis['on_time_pct']) else "—")
    c3, c4 = st.columns(2)
    c3.metric("Median LT (d)", f"{live_kpis['lead_time_median']:.1f}" if not math.isnan(live_kpis['lead_time_median']) else "—")
    c4.metric("P90 LT (d)", f"{live_kpis['lead_time_p90']:.1f}" if not math.isnan(live_kpis['lead_time_p90']) else "—")
    figL = make_gantt(live_events, ref_date)
    if figL is not None:
        st.plotly_chart(figL, use_container_width=True, key=f"plot_live_{live_key}")

with colR:
    st.markdown("**Full (exact inputs)**")
    c1, c2 = st.columns(2)
    c1.metric("Throughput", f"{full_kpis['throughput']}")
    c2.metric("On‑Time (%)", f"{full_kpis['on_time_pct']:.0f}%" if not math.isnan(full_kpis['on_time_pct']) else "—")
    c3, c4 = st.columns(2)
    c3.metric("Median LT (d)", f"{full_kpis['lead_time_median']:.1f}" if not math.isnan(full_kpis['lead_time_median']) else "—")
    c4.metric("P90 LT (d)", f"{full_kpis['lead_time_p90']:.1f}" if not math.isnan(full_kpis['lead_time_p90']) else "—")
    figR = make_gantt(full_events, ref_date)
    if figR is not None:
        st.plotly_chart(figR, use_container_width=True, key=f"plot_full_{full_key}")

# Delta table
st.subheader("Delta (Full − Live)")
keys = ['throughput', 'lead_time_median', 'lead_time_p90', 'on_time_pct']
delta = {k: (full_kpis[k] - live_kpis[k]) if not (math.isnan(full_kpis[k]) or math.isnan(live_kpis[k])) else float('nan') for k in keys}
delta_df = pd.DataFrame([
    {'Metric': 'Throughput', 'Live': live_kpis['throughput'], 'Full': full_kpis['throughput'], 'Delta (Full−Live)': delta['throughput']},
    {'Metric': 'Lead Time Median (d)', 'Live': live_kpis['lead_time_median'], 'Full': full_kpis['lead_time_median'], 'Delta (Full−Live)': delta['lead_time_median']},
    {'Metric': 'Lead Time P90 (d)', 'Live': live_kpis['lead_time_p90'], 'Full': full_kpis['lead_time_p90'], 'Delta (Full−Live)': delta['lead_time_p90']},
    {'Metric': 'On‑Time (%)', 'Live': live_kpis['on_time_pct'], 'Full': full_kpis['on_time_pct'], 'Delta (Full−Live)': delta['on_time_pct']}
])
st.dataframe(delta_df, use_container_width=True)

# Details expanders
with st.expander("Live: Event Log & Lead Times"):
    st.dataframe(live_events.sort_values(['patient_id','start_day']).reset_index(drop=True), use_container_width=True)
    st.dataframe(live_lt.sort_values('patient_id').reset_index(drop=True), use_container_width=True)
with st.expander("Full: Event Log & Lead Times"):
    st.dataframe(full_events.sort_values(['patient_id','start_day']).reset_index(drop=True), use_container_width=True)
    st.dataframe(full_lt.sort_values('patient_id').reset_index(drop=True), use_container_width=True)

# Exports
st.subheader("Export Results")
colE1, colE2 = st.columns(2)
with colE1:
    st.download_button("Download Live Event Log CSV", live_events.to_csv(index=False).encode('utf-8'), file_name="events_live.csv", mime="text/csv")
    st.download_button("Download Live Lead Times CSV", live_lt.to_csv(index=False).encode('utf-8'), file_name="lead_times_live.csv", mime="text/csv")
with colE2:
    st.download_button("Download Full Event Log CSV", full_events.to_csv(index=False).encode('utf-8'), file_name="events_full.csv", mime="text/csv")
    st.download_button("Download Full Lead Times CSV", full_lt.to_csv(index=False).encode('utf-8'), file_name="lead_times_full.csv", mime="text/csv")

st.caption("Live = capped patients & horizon for responsiveness. Full = exact parameters. Use the delta table to teach approximation vs exact.")
