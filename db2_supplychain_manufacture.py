# app_v2.py — CGT Supply Chain Simulator (Autologous with Manufacturing Substeps)
# ---------------------------------------------------------------------------------
# New in v2:
# - Expands Manufacturing into three substeps: Activation → Transduction → Expansion
# - Optional vector-availability delays before Transduction (probability + delay distribution)
# - Separate duration controls for each substep; shared manufacturing rig capacity
# - KPI cards unchanged; Gantt now shows substeps explicitly
# - CSV export of event log, lead-time table, and KPIs
# ---------------------------------------------------------------------------------
# D. Bulseco, Institute for Future Intelligence, 2025 with manufacturing (v 0.01)
...
class CGTSim:
    def __init__(self, p: ScenarioParams):
        ...
        self.mfg = simpy.Resource(self.env, capacity=p.mfg_rigs)  # shared across activation/transduction/expansion
    ...
    def patient_flow(self, idx: int):
        ...
        # Manufacturing substeps (share mfg rigs)
        # Activation
        yield self.env.process(self._do_step(
            self.mfg,
        ...
        # Optional vector delay BEFORE transduction (e.g., waiting for vector lot)
...
st.set_page_config(page_title="CGT Simulator v2 — Autologous Substeps", layout="wide")
st.title("Cell & Gene Therapy Supply Chain Simulator — Autologous v2")
...
with st.sidebar:
    ...
    apheresis_cap = st.slider("Apheresis Chairs", 1, 10, 3)
    mfg_rigs = st.slider("Manufacturing Rigs", 1, 12, 3)
    qc_stations = st.slider("QC Stations", 1, 12, 2)
    clinic_slots = st.slider("Clinic Infusion Slots / day", 1, 24, 6)
    ...
    act_mean = st.number_input("Activation mean", 0.1, 10.0, 1.0, step=0.1)
    act_sd = st.number_input("Activation sd", 0.0, 5.0, 0.2, step=0.1)

    trans_mean = st.number_input("Transduction mean", 0.1, 10.0, 0.5, step=0.1)
    trans_sd = st.number_input("Transduction sd", 0.0, 5.0, 0.1, step=0.1)

    exp_mean = st.number_input("Expansion mean", 0.5, 30.0, 5.5, step=0.5)
    exp_sd = st.number_input("Expansion sd", 0.0, 10.0, 1.0, step=0.5)

    ff_mean = st.number_input("Fill/Finish mean", 0.05, 5.0, 0.5, step=0.05)
    ff_sd = st.number_input("Fill/Finish sd", 0.0, 2.0, 0.1, step=0.05)
    ...
    rel_mean = st.number_input("Release mean", 0.01, 5.0, 0.25, step=0.05)
    rel_sd = st.number_input("Release sd", 0.0, 2.0, 0.05, step=0.05)
    ...
    inf_mean = st.number_input("Infusion mean", 0.01, 2.0, 0.2, step=0.05)
    ...
    vec_mean = st.number_input("Delay mean (days)", 0.0, 20.0, 2.0, step=0.5)
    vec_sd = st.number_input("Delay sd (days)", 0.0, 10.0, 0.5, step=0.5)

    st.subheader("Targets & Reproducibility")
    target_lt = st.slider("On-Time Lead Time Target (days)", 7, 120, 28)
...
with col_tip:
    st.caption("Try increasing vector delay probability to see bottlenecks shift from QC to pre-Transduction")
...
if run_sim:
    ...
    kpi_df = pd.DataFrame([{**kpis, **{'qc_days': params.qc_days, 'qc_method': qc_mode, 'target_lead_time_days': params.target_lead_time_days,
                                        'vector_delay_prob': params.vector_delay_prob,
                                        'vector_delay_mean': params.vector_delay_mean,
                                        'vector_delay_sd': params.vector_delay_sd}}])
    kpi_csv = kpi_df.to_csv(index=False).encode('utf-8')

    d1, d2, d3 = st.columns(3)
    with d1:
        st.download_button("Download Event Log CSV", events_csv, file_name="events_v2.csv", mime="text/csv")
    with d2:
        st.download_button("Download Lead Times CSV", lt_csv, file_name="lead_times_v2.csv", mime="text/csv")
    with d3:
        st.download_button("Download KPI Summary CSV", kpi_csv, file_name="kpis_v2.csv", mime="text/csv")

else:
    st.info("Adjust parameters in the sidebar and click **Run Simulation**. Manufacturing now shows Activation → Transduction → Expansion.")

st.divider()
st.markdown(
    """
    **Notes**
    - Manufacturing substeps share the same cleanroom/rig capacity to model real-world suite utilization.
    - *Vector Wait* models upstream vector unavailability before Transduction (probability + delay days).
    - QC duration reflects USP <71> (14 d) vs validated rapid methods (e.g., 3 d), or set a custom value.
    - Clinic slots are scheduled in daily buckets; excess demand pushes infusions to later days.
    """
)
